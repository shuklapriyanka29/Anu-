...
const win =window.sessionStorage;
  ...
  const handlestatusoftask=() =>{
    alert("------Handle status of task---------");
  }
  const menu = (
    <Menu>
  ...
  useEffect(() => {
    ...
    const win =window.sessionStorage;
      ...
      var url1 = PMbaseURL+"getTestingdata?empdata=OMI-0076";
      fetch(url1)
      // https://demo.omfysgroup.com/project-managementQpotAPi/qpot/getTestingdata?empdata=OMI-0076


  // fetch("https://demo.omfysgroup.com/project-managementQpotAPi/qpot/getTestingdata")
    ...
    .then((result) => {
      ...
      const rows = result.map((report) => ({
          ...
          testingassignedto:report.tester_name,
          testingrequestdate:report.creation_date_trans,
          priority:report.priority,
          needbydate:report.need_by_date_trans,
          attachment: <Button variant="text" size="small" type="button" className="btn btn-primary" data-toggle="modal" data-target="#exampleModal" onClick={event => fillChilddata(report.tr_id)}><AttachmentIcon style={{fontSize: '20px'}}/></Button>,                                                              
         
       taskstatus:<Button variant="text" size="small" type="button" className="btn btn-primary"  onClick={handlestatusoftask}>Completed</Button>,
  ...
  const fillChilddata = (tr_id) =>{
    ...
    // alert(tr_id)
      ...
      const win =window.sessionStorage;
        const value = win.getItem('username');
        var url2 = PMbaseURL+"getTestingdatabyheaderid?tr_id="+tr_id;
        fetch(url2)
    // fetch("https://demo.omfysgroup.com/project-managementQpotAPi/qpot/getTestingdatabyheaderid?tr_id="+tr_id)
      ...
      // alert("=====empid======"+value)
        const rows = result.map((report,index) => ({
          ...
          //  attachment: (
          // <a href={"https://demo.omfysgroup.com/SpringBootRestAPIMongoDB1/uploadTestingFile/"+report.object_Storage_Id} style={{color:'black !important'}}><DownloadIcon style={{color:'black !important'}} sx={{ fontSize: 15 }}/></a>),                                                              
          attachment:( 
